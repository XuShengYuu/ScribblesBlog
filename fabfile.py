...
env.roledefs = {
    'myserver': ['ubuntu@203.195.180.21:22'],
}
env.PROJECT_NAME = 'Scribbles'
env.SETTINGS_BASE = 'scribbles/scribbles/scribbles/settings/base.py'
env.DEPLOY_PATH = '/home/scribbles/venvs/scribbles-env/'
...
class _Version:
    ...
    def replace(self, f, version):
        with open(f, 'r',encoding='gb18030',errors='ignore') as fd:
            origin_content = fd.read()
            content = origin_content.replace('${version}', version)

        with open(f, 'w',encoding='gb18030',errors='ignore') as fd:
            fd.write(content)
...
def _ensure_virtualenv():
    ...
    if not exists(env.DEPLOY_PATH):
        run('sudo mkdir -p %s/tmp' % env.DEPLOY_PATH)

    run('sudo python3.7 -m venv %s' % env.DEPLOY_PATH)
